...
const SerachPatient = ({create}) => {
    ...
    const {title,close} = useSelector((state) => state?.modalSlice?.data)
    ...
    return (
        <div className=" h-full w-full text-sm   flex justify-center items-center shadow-sm">
            <div className={` p-5 overflow-scroll flex-col  xl:w-[60%]  w-[50%]       text-lightModalHeaderColor flex items-center justify-center  bg-white   border-lightBorderColor  dark:bg-darkSecondary dark:border-b dark:border-darkSecondary `}>
            <div className="   w-full flex justify-between mb-4  border-opacity-50  ">
                        <h2 className="capitalize  tracking-wider  font-semibold   text-lightModalHeaderColor dark:text-darkTitleColor">
                            {title}
                        </h2>
                ...
                <div className=" p-4 rounded-xl bo rder gap-4 border- border-gray-500 border-opacity-50  flex flex-col w-full">
                    
                    <div className="w-full    gap-4 flex    dark:text-darkTitleColor    rounded      ">
                        <div className="w-1/2 flex justify-center text-sm items-center flex-col gap-4  border border-gray-500 border-opacity-50 rounded-md   p-5     ">
                            ...
                            {currentUser?.role?.id == 1 || currentUser?.role?.id == 2 || currentUser?.role?.id == 3 || currentUser?.role?.id == 15 || !currentUser?.branch ? <div className="w-full mb-4 ">
                                <label className=''>
                                    <p className={`flex gap-4 mb-1 ${isDark ? "text-white" : "text-black"}`}>
                    ...
                    <div className="w-full      flex justify-end items-center  gap-4    ">
                        <SPKBTNCancel text="Close" onClick={closeModal} />
                        {
                            formData?.patientId?.length && formData?.branchId?.length ?

                                <SPKBTNNew text='Create' onClick={()=>{create(selectedPatient);closeModal()}}/>
                                 :
                                ''
                        }
                    </div>
